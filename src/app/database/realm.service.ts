...
export class RealmService implements DatabaseLink {
  ...
  private convertIdStringsToObjectIds<T extends Model>(model: T): T {
    const converted: T = {
      ...
      account: new ObjectId(model.account as string),
    }
  ...
  fetch = {
    account: (_id: Account['_id']) => this.util.promiseToObservable(() => this.app.currentUser.functions.fetchAccount(new ObjectId(_id))),
    ...
    gradingGroup: (_id: GradingGroup['_id']) => this.util.promiseToObservable(
      () => this.app.currentUser.functions.fetchGradingGroup(new ObjectId(_id)),
    ),
  }
  put = {
    ...
    account: (account: Account) => this.util.promiseToObservable(
      () => this.app.currentUser.functions.putAccount(this.convertIdStringsToObjectIds(account)),
    ),
    ...
    gradingGroup: (gradingGroup: GradingGroup) => this.util.promiseToObservable(
      () => this.app.currentUser.functions.putGradingGroup(this.convertIdStringsToObjectIds(gradingGroup)),
    ),
  }
  all = {
    ...
    gradingGroups: (courseId: Course['_id']) => this.util.promiseToObservable(() => this.app.currentUser.functions.fetchGradingGroups(new ObjectId(courseId))),
  }
  remove = {
    ...
    gradingGroup: (_id: GradingGroup['_id']) => this.util.promiseToObservable(
      () => this.app.currentUser.functions.removeGradingGroup(new ObjectId(_id)),
    ),
  }
  search = {
